"""
Models for problems in the Development of high-intensity focused ultrasound
simulator for large-scale computing (2011) article by Kohei Okita, Kenji Ono,
Shu Takagi and Yoichiro Matsumoto.
"""
...
from dolfinx.fem import FunctionSpace, Function, assemble_vector, form, assemble_matrix
from ufl import FiniteElement, TestFunction, Measure, inner, grad, ds, dx, dot, FacetNormal, TrialFunction


class PeriodicWavePropagation:
	"""
	Solver for section 4.1 in the paper.
	
	This solver uses GLL lattice and GLL quadrature.
	"""

	def __init__(self, mesh, meshtags, degree, rho0, c0, freq0, p0):
		self.mesh = mesh
		normal = FacetNormal(mesh)

		FE = FiniteElement("Lagrange", mesh.ufl_cell(), degree, variant="gll")
		...
		self.L = form(self.c0**2*(- inner(grad(self.u_n), grad(self.v))
                                  * dx(metadata=md)
                                  + inner(dot(grad(self.g), normal), self.v)
                                  * ds(1, metadata=md)))
	...
	def f1(self, t: float, u: PETSc.Vec, v: PETSc.Vec, result: PETSc.Vec):
		...
		# Update source
		self.g.interpolate(lambda x: self.rho0 * self.c0 
							* np.cos(self.w0 * t) *
							(np.sin(self.k*x[0]) + np.sin(self.k*x[1])))
												#  + np.sin(self.k*x[2])))
...
class PeriodicWavePropagationExact:
	"""
	Solver for section 4.1 in the paper.
	...
	def __init__(self, mesh, meshtags, degree, rho0, c0, freq0, p0):
		...
		FE = FiniteElement("Lagrange", mesh.ufl_cell(), degree, variant="equispaced")
		self.V = FunctionSpace(mesh, FE)
		self.v = TestFunction(self.V)
