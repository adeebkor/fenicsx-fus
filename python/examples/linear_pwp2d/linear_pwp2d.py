...
# .. _linear_pwp2d:
#
# Linear solver for the 2D periodic wave propagation problem
# ==========================================================
...
from dolfinx.mesh import (MeshTags, CellType, create_rectangle, 
                          locate_entities_boundary)
from ufl import FiniteElement, inner, dx

from hifusim import PeriodicWavePropagation
...
# Tag boundaries
facets0 = locate_entities_boundary(mesh, 1, lambda x: np.isclose(x[0], 0.0))
facets1 = locate_entities_boundary(mesh, 1, lambda x: np.isclose(x[1], 0.0))
facets2 = locate_entities_boundary(mesh, 1, lambda x: np.isclose(x[0], L))
facets3 = locate_entities_boundary(mesh, 1, lambda x: np.isclose(x[1], L))
...
mt = MeshTags(mesh, 1, indices, values[pos])
...
eqn.u_n.interpolate(lambda x: rho0*c0*(np.sin(x[0]*k) + np.sin(x[1]*k)))
eqn.v_n.x.array[:] = 0.0
...
with XDMFFile(MPI.COMM_WORLD, "u_exact.xdmf", "w") as file:
	file.write_mesh(mesh)
	file.write_function(u_exact)

with XDMFFile(MPI.COMM_WORLD, "u_fe.xdmf", "w") as file:
	file.write_mesh(mesh)
	file.write_function(eqn.u_n)
