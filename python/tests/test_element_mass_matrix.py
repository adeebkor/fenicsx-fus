...
def test_diagonal(dimension, p):
    ...
    if dimension == 1:
        # Interval mesh
        mesh = dolfinx.mesh.create_interval(MPI.COMM_WORLD, 1, [-1, 1])
    ...
    md = {"quadrature_rule": "GLL", "quadrature_degree": qd[str(p)]}
    ...
    assert np.allclose(idx[0], np.arange(ndof)) and np.allclose(idx[1], np.arange(ndof))
