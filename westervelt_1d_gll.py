...
from models import Westervelt1D, Westervelt1DGLL
...
L2_diff = mesh.mpi_comm().allreduce(
    assemble_scalar(inner(diff, diff) * dx(
		metadata={"quadrature_degree": 2*(degree+3)+1})), op=MPI.SUM)
L2_exact = mesh.mpi_comm().allreduce(
    assemble_scalar(inner(u_e, u_e) * dx(
		metadata={"quadrature_degree": 2*(degree+3)+1})), op=MPI.SUM)
