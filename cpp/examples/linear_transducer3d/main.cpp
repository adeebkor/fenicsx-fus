//
// Linear solver for the 3D transducer problem
// ===========================================
...
int main(int argc, char* argv[]) {
  ...
  common::subsystem::init_mpi(argc, argv);
  {
    ...
    auto mesh
        = std::make_shared<mesh::Mesh>(xdmf.read_mesh(element, mesh::GhostMode::none, "transducer3d"));
    mesh->topology().create_connectivity(1, 2);
    auto mt = std::make_shared<mesh::MeshTags<std::int32_t>>(
        xdmf.read_meshtags(mesh, "transducer3d_boundaries"));
    ...
    double finalTime = 1.5 * (domainLength / speedOfSound); // + 8.0 / sourceFrequency;
