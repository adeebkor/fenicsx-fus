...
template <int P>
class LossyGLLHeterogenous {
public:
  LossyGLLHeterogenous(std::shared_ptr<mesh::Mesh> Mesh,
    ...
    L = std::make_shared<fem::Form<double>>(
        ...
        {{"u_n", u_n}, {"g", g}, {"v_n", v_n}, {"dg", dg}, {"c0", c0}, {"delta", delta}},
        {},
        {{dolfinx::fem::IntegralType::exterior_facet, &(*mt_facets)}}));
...
private:
  ...
  std::shared_ptr<fem::Function<double>> c0, delta, g, dg, u, v, u_n, v_n;
  std::shared_ptr<fem::Form<double>> a, L;
  std::shared_ptr<la::Vector<double>> m, b; 

  xtl::span<double> _c0, _delta, _g, _dg, out;
