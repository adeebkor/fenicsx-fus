//
// Lossy solver for the 3D planar problem in heterogenous domain (spectral)
// ========================================================================
// Copyright (C) 2022 Adeeb Arif Kor

#include "LossyGLLSpectralHeterogenous.hpp"
...
double computeDiffusivityOfSound(double& frequency, double& speed, double& attenuationdB) {
  double attenuationNp = attenuationdB / 20 * log(10);  // (Np/m/MHz^2)
  double diffusivity = 2 * attenuationNp * speed * speed * speed / frequency / frequency;
  return diffusivity;
}
...
int main(int argc, char* argv[]) {
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    double period = 1.0 / sourceFrequency;                // (s)
    double angularFrequency = 2 * M_PI * sourceFrequency; // (rad / s)

    // Material parameters
    double speedOfSoundWater = 1500.0;  // (m/s)
    double attenuationCoefficientWaterdB = 0.0;  // (dB/m)
    double diffusivityOfSoundWater = computeDiffusivityOfSound(angularFrequency, speedOfSoundWater, attenuationCoefficientWaterdB);

    double speedOfSoundBone = 2800.0;   // (m/s)
    double attenuationCoefficientBonedB = 400.0;  // (dB/m)
    double diffusivityOfSoundBone = computeDiffusivityOfSound(angularFrequency, speedOfSoundBone, attenuationCoefficientBonedB);
    ...
    if (rank == 0) {
      ...
      std::cout << "Speed of sound (water): " << speedOfSoundWater << std::endl;
      std::cout << "Speed of sound (bone): " << speedOfSoundBone << std::endl;
      std::cout << "Sound diffusivity (water): " << diffusivityOfSoundWater << std::endl;
      std::cout << "Sound diffusivity (bone): " << diffusivityOfSoundBone << std::endl;
    ...
    instantiation.start();
    auto eqn = LossyGLLSpectralHeterogenous<degreeOfBasis>(
        mesh, mt_cells, mt_facets, speedOfSoundWater, speedOfSoundBone,
        diffusivityOfSoundWater, diffusivityOfSoundBone,
