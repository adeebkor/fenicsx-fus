//
// Lossy solver for the 3D planar problem (spectral)
// =================================================
// Copyright (C) 2022 Adeeb Arif Kor

#include "LossyGLLSpectral.hpp"
...
int main(int argc, char* argv[]) {
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    double attenuationCoefficientdB = 100;  // (dB/m)
    double attenuationCoefficientNp = attenuationCoefficientdB / 20 * log(10); // (Np/m/MHz^2)
    double diffusivityOfSound = 2 * attenuationCoefficientNp 
                                * speedOfSound * speedOfSound * speedOfSound
    ...
    instantiation.start();
    auto eqn = LossyGLLSpectral<degreeOfBasis>(
        mesh, mt, speedOfSound, diffusivityOfSound, sourceFrequency, 
        pressureAmplitude);
    instantiation.stop();
