//
// This code simulates a 3D viscoelastic wave problem with planar transducer 
// source. The simulation is conducted to collect data that will be use for
// comparison with lossy and Westervelt problem.
// ==========================================================================
...
int main(int argc, char* argv[])
{
...

  {
    ...
    // Source parameters
    const T sourceFrequency = 1e6;  // (Hz)
    const T sourceAmplitude = 125000;  // (Pa)
    const T period = 1 / sourceFrequency;  // (s)
    const T angularFrequency = 2 * M_PI * sourceFrequency;  // (rad/s)

    // Material parameters
    const T speedOfSound = 1482.32;  // (m/s)
    const T density = 998.2;  // (kg/m^3)
    const T attenuationCoefficientdB = 0.217;  // (dB/m)
    ...
    "/home/mabm4/rds/hpc-work/mesh/S1/planar_3d_0/mesh.xdmf", "r");
    ...
    if (mpi_rank == 0){
      ...
      std::cout << "Source: Circular planar" << "\n";
