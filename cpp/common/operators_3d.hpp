...
class MassOperator {
  ...
  public:
    MassOperator(std::shared_ptr<fem::FunctionSpace>& V, int bdegree) : _dofmap(0) {
      ...
      auto jacobian_data = precompute_jacobian_data_hex(mesh, bdegree);
...
//       }
//       const double fw0 = std::pow(1486.0, 2) * (G(iq, 1, 0) * w0 + G(iq, 1, 1) * w1 + G(iq, , ) * w2);
//       const double fw1 = std::pow(1486.0, 2) * (G(iq, 0, 0) * w0 + G(iq, 0, 1) * w1 + G(iq, , ) * w2);
//       const double fw2 = std::pow(1486.0, 2) * (G(iq, ,) * w0 + G(iq, , ) * w1 + G(iq, , ) * w2);
//       for (int i = 0; i < nd; i++){
