...
template <int P>
class LossyGLLSpectral {
public:
  LossyGLLSpectral(std::shared_ptr<mesh::Mesh> Mesh,
    ...
    double coeff0 = - 1.0 * (c0_ * c0_);
    double coeff1 = - 1.0 * (delta_);
    stiff_c0 = std::make_shared<StiffnessSpectral<double, P>>(V, coeff0);
    stiff_delta = std::make_shared<StiffnessSpectral<double, P>>(V, coeff1);
...
private:
  ...
  std::shared_ptr<StiffnessSpectral<double, P>> stiff_c0, stiff_delta;
