...
int main(int argc, char* argv[]) {
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    const T sourceAmplitude = density * speedOfSound * sourceSpeed; // (Pa)
    const T period = 1 / sourceFrequency; // (s)

    // Domain parameters
    const T wavelength = speedOfSound / sourceFrequency;
    const T scattererRadius = 0.5 * wavelength;
    const T domainScale = 11;
    const T domainLength = 1.5 * domainScale * wavelength + 2 * scattererRadius; // (m)
    ...
    auto mesh = std::make_shared<mesh::Mesh<T>>(
        fmesh.read_mesh(coord_element, mesh::GhostMode::none, "circular_scatterer_soundhard_2d_2"));
    mesh->topology()->create_connectivity(1, 2);
    auto mt_cell = std::make_shared<mesh::MeshTags<std::int32_t>>(
        fmesh.read_meshtags(*mesh, "circular_scatterer_soundhard_2d_2_cells"));
    auto mt_facet = std::make_shared<mesh::MeshTags<std::int32_t>>(
        fmesh.read_meshtags(*mesh, "circular_scatterer_soundhard_2d_2_facets"));
