...
int main(int argc, char* argv[]) {
...

  {
    ...
    double timeStepSize
        = CFL * meshSizeMinGlobal / (speedOfSoundBone * degreeOfBasis * degreeOfBasis);
    const int stepPerPeriod = period / timeStepSize + 1;
    timeStepSize = period / stepPerPeriod;
    const double startTime = 0.0;
    const double finalTime = domainLength / speedOfSoundWater + 4.0 / sourceFrequency;
    ...
    // Model
    auto model = LinearSpectral2D<double, degreeOfBasis>(mesh, mt_facet, c0, rho0, sourceFrequency,
    ...
    // Check norms
    auto Norm = std::make_shared<fem::Form<double>>(
        fem::create_form<double>(*form_forms_Norm, {}, {{"u_n", u_n}}, {}, {}, mesh));
