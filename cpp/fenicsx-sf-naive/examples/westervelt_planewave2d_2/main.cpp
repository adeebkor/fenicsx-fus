...
int main(int argc, char* argv[]) {
...

  {
    ...
    // Source parameters
    const double sourceFrequency = 1e6;                         // (Hz)
    const double sourceAmplitude = 125000;                      // (Pa)
    const double period = 1 / sourceFrequency;                  // (s)
    const double angularFrequency = 2 * M_PI * sourceFrequency; // (rad/s)

    // Material parameters
    const double speedOfSound = 1482.32; // (m/s)
    const double density = 998.2;        // (kg/m^3)
    const double nonlinearCoefficient = 3.5;
    const double attenuationCoefficientdB = 0.217; // (dB/m)
    ...
    // Model
    auto model = WesterveltSpectral2D<double, 4>(mesh, mt_facet, c0, rho0, delta0, beta0,
