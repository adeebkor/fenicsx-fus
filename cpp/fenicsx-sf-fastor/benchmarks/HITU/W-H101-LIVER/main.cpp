...
int main(int argc, char* argv[]) {
...

  {
    ...
    // Source parameters
    const T speedOfSound = 1590.0;                                     // (m/s)
    const T density = 1060.0;                                          // (kg/m^3)
    const T sourceFrequency = 1.1e6;                                   // (Hz)
    const T sourceVelocity = 0.1853331526917232;                       // (m/s)
    ...
    // Material parameters
    const T nonlinearCoefficient = 4.4;
    const T attenuationCoefficientdB = 90.0; // (dB/m)
    ...
    io::XDMFFile fmesh(MPI_COMM_WORLD, "/home/mabm4/rds/hpc-work/mesh/HITU/H101/mesh.xdmf", "r");
